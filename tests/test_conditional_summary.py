...
def test_update_if_contains():
    ...
    print(
        f"[O] Would skip: {current_tag_v2} (does not contain '{config.update_if_contains}')"
    )
...
def test_skip_if_contains():
    ...
    print(
        f"[O] Would update: {current_tag_version} (does not contain '{config.skip_if_contains}')"
    )
...
def test_combined_conditions():
    ...
    for tag, should_update, reason in test_cases:
        matches_update = (
            not config.update_if_contains or config.update_if_contains in tag
        )
        ...
        assert result == should_update, (
            f"Failed for {tag}: expected {should_update}, got {result}"
        )
